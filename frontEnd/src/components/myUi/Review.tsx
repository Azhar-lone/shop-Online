      ...
      <div className="flex justify-between md:w-[60%]">
        ...
        <div className="flex gap-5 flex-col">
          <Avatar className="size-16">
            <AvatarFallback>{review.owner.firstName.charAt(0)}</AvatarFallback>
            <AvatarImage
              src={review.owner.profilePic}
              onClick={() => navigate("/" + review.owner.userName)}
            />
          </Avatar>
          <div>
            ...
            <h1 className="font-serif ">{review.createdAt.toLocaleString()}</h1>
            {review.createdAt !== review.updatedAt && (
              <h1 className="font-serif ">edited</h1>
            )}
          </div>
        </div>
        {/* Stars */}
