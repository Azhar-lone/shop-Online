...
const CartPage: React.FC = () => {
  ...
  function CalculateTatalPrice() {
    cart.forEach((cartItem) => {
      return setTotalPrice((prev) => cartItem.price + prev);
    });
  }
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="hidden md:table-cell">Picture</TableHead>
            <TableHead className=" md:table-cell">Inclusion Date</TableHead>
            <TableHead className=" md:table-cell">Price</TableHead>
            <TableHead className=" md:table-cell">Remove</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {cart.map((cartItem, i: number) => (
            ...
            >
              ...
              <TableCell className="md:table-cell w-[10%]  hidden h-10">
                <img src={cartItem.img[0]} alt="" />
              </TableCell>
              <TableCell>{cartItem.createdAt.toDateString()}</TableCell>
              <TableCell>
                <>
                  {"$"}
                  {cartItem.price}
