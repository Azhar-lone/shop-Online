  ...
  async function onSubmit(values: z.infer<typeof loginSchema>) {
    try {
      ...
      let response = await fetch(
        import.meta.env.VITE_BackendUrl + baseUrl + "/users/login",
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    <div className="md:w-[60%] w-[100%] mx-auto p-5 flex flex-col gap-5  bg-background shadow-2xl shadow-primary mt-[5vh] animate-accordion-down ">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-between w-[100%]">
            ...
            <Button
              variant="ghost"
              onClick={() =>
                navigate("/forgot-password", { state: form.getValues("email") })
              }
            >
              Forgot Password
            </Button>
      ...
      <div>
        ...
        <Link to={"/signup"} className="text-blue-500 p-2 hover:text-blue-400">
          SignUp
        </Link>
      </div>
    </div>
